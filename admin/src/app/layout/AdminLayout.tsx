...
const items = [
  { key: "/dashboard", activePrefix: "/dashboard", label: "看板数据", shortLabel: "看板", description: "核心经营指标与趋势", icon: <LayoutDashboard size={18} /> },
  { key: "/orders", activePrefix: "/orders", label: "订单运营", shortLabel: "运营", description: "备餐、固定订餐、订单处理", icon: <ClipboardList size={18} /> },
  { key: "/dispatch", activePrefix: "/dispatch", label: "骑手中心", shortLabel: "骑手", description: "派单、轨迹、回执与通知", icon: <Bike size={18} /> },
  { key: "/aftersales", activePrefix: "/aftersales", label: "售后台账", shortLabel: "售后", description: "登记、结算、补偿与退款对账", icon: <LifeBuoy size={18} /> },
    ...
    <div className="admin-shell" style={scaleStyle}>
      ...
      <div className="sidebar">
        ...
        <div className="nav-menu">
          {items.map((item) => {
            const active = location.pathname.startsWith(item.activePrefix);
            return (
            <Link
              key={item.key}
              to={item.key}
            ...
            >
              ...
              <span style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                <span style={{ fontSize: "11px", color: active ? "rgba(255,255,255,0.85)" : "var(--text-muted)", fontWeight: 500 }}>
                  {item.description}
                </span>
              </span>
            </Link>
      ...
      <div className="mobile-quick-actions">
        <button onClick={() => navigate("/dispatch/progress")}>查看配送进度</button>
        <button onClick={() => navigate("/customers")}>前往临时加餐</button>
      </div>
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <SafeInput
                  ...
                  onValueChange={(value) => setPasswordForm((current) => ({ ...current, oldPassword: value }))}
                  placeholder="请输入当前密码"
                />
              </div>
              <div className="form-group">
                ...
                <SafeInput
                  ...
                  onValueChange={(value) => setPasswordForm((current) => ({ ...current, newPassword: value }))}
                  placeholder="请输入新密码"
                />
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <SafeInput
                  ...
                  onValueChange={(value) => setPasswordForm((current) => ({ ...current, confirmPassword: value }))}
                  placeholder="再次输入新密码"
                />
              </div>
            </div>
