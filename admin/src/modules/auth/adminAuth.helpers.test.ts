...
describe("buildSavedAdminCredentials", () => {
  it("persists only the phone for remembered login (never plaintext password)", () => {
    expect(
      buildSavedAdminCredentials({
        phone: " 17671863805 ",
...
describe("parseSavedAdminCredentials", () => {
  ...
  it("returns null for malformed remembered credentials payload", () => {
    ...
    expect(parseSavedAdminCredentials(null)).toBeNull();
  });
});
...
describe("buildInitialAdminLoginForm", () => {
  ...
  it("rehydrates the remembered phone while keeping the password empty", () => {
