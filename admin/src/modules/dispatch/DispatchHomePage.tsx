...
export function DispatchHomePage() {
  ...
  const reloadAll = useCallback(async () => {
    ...
    if (!isPromiseFulfilledResult(ovResult)) failedSections.push("概览");
    if (!isPromiseFulfilledResult(bindingsResult)) failedSections.push("区域");
    if (!isPromiseFulfilledResult(pendingResult)) failedSections.push("待分配订单");
    ...
    setReloadWarning(
      failedSections.length > 0
        ? `部分分单数据刷新失败：${failedSections.join("、")}。页面当前展示的是最近一次成功获取的数据。`
        : ""
    );
    setLoadingAll(false);
