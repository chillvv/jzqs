      ...
      <div className="menu-slot-editor">
        ...
        <div className="form-group">
          ...
          {slot.dishItems.map((dish, index) => (
            <div key={`${mealPeriod}-${index}`} className="menu-slot-editor__dish-row">
              <input className="form-control" value={dish} onChange={(e) => updateDishItem(serveDate, mealPeriod, index, e.target.value)} placeholder={`第 ${index + 1} 道菜`} />
        ...
        <div className="menu-slot-editor__split">
          <div className="form-group menu-slot-editor__field">
            ...
            <input className="form-control" type="number" value={slot.totalCalories ?? ""} onChange={(e) => updateSlot(serveDate, mealPeriod, "totalCalories", e.target.value ? Number(e.target.value) : null)} />
          </div>
          <div className="form-group menu-slot-editor__field">
            <RemarkField
              label="备注"
  ...
  return (
    <>
      ...
      <div className="table-container menu-week-board">
        {loading && <div className="empty-state">菜单加载中...</div>}
        {!loading && !week?.days.length && <div className="empty-state">当前没有可编辑的周菜单</div>}
        {!loading && week?.days.map((day: AdminMenuWeekDay) => {
      ...
      {isPublishConfirmOpen && (
        <div className="modal-overlay" onClick={() => setIsPublishConfirmOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{publishConfirmTitle}</h3>
              <button className="modal-close" onClick={() => setIsPublishConfirmOpen(false)}>✕</button>
            </div>
