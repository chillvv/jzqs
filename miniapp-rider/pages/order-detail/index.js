...
Page({
  ...
  async loadReferenceImage(options = {}) {
    ...
    try {
      const result = await taskService.getAddressReferenceImage(order.addressId);
  ...
  async deleteSubmittedReceipt() {
    ...
    try {
      await taskService.deleteReceiptImage(order.mealSlotOrderId, order.receiptUrl);
  ...
  async handleSubmitReceipt() {
    ...
    try {
      ...
      if (receiptTempFilePath && !isStoredReceiptReference(receiptTempFilePath)) {
        ...
        const uploadResult = await taskService.uploadReceipt(receiptTempFilePath);
  ...
  async handleUndoDelivery() {
    ...
    try {
      await taskService.undoDelivery(order.mealSlotOrderId);
  ...
  async handleDefer() {
    ...
    try {
      await taskService.deferQueueItem(order.batchItemId);
