...
/**
 * 获取今日任务概览
 * @param {string} serveDate - 工作台日期
 * @returns {Promise<Object>} 任务概览数据
 */
async function getTodaySummary(serveDate) {
  return await request({
    url: appendServeDateQuery(
      '/api/mobile/rider/summary',
      serveDate
    )
  });
}
 ...
 */
async function getQueue(serveDate) {
  return await request({
    url: appendServeDateQuery(
 ...
 * @param {string} serveDate - 工作台日期
 * @param {number} mealSlotOrderId - 订单ID，用于回退查询
 * @returns {Promise<Object>} 订单详情
 */
async function getOrderDetail(batchItemId, serveDate, mealSlotOrderId) {
  const requestId = resolveQueueItemRequestId(batchItemId, mealSlotOrderId);
  let url = `/api/rider/orders/${requestId}`;
...
async function getCompletedToday() {
  return await request({
    url: '/api/mobile/rider/completed-today'
  });
}
 ...
 */
async function reorderQueue(batchItemIds) {
  return await request({
    url: '/api/mobile/rider/queue/reorder',
    method: 'POST',
    data: { batchItemIds },
 ...
 * @returns {Promise<Object>} 操作结果
 */
async function deferQueueItem(batchItemId) {
  return await request({
    url: `/api/mobile/rider/queue/items/${batchItemId}/defer`,
    method: 'POST'
  });
}
 ...
 * @returns {Promise<Object>} 操作结果
 */
async function resumeQueueItem(batchItemId) {
  return await request({
    url: `/api/mobile/rider/queue/items/${batchItemId}/resume`,
    method: 'POST'
  });
}
 ...
 */
async function submitReceipt(mealSlotOrderId, receiptFileKey, receiptNote, deliveredAt) {
  return await request({
 ...
 */
async function updateReceipt(mealSlotOrderId, receiptFileKey, receiptNote, deliveredAt) {
  return await request({
    url: `/api/rider/orders/${mealSlotOrderId}/receipt`,
    method: 'PUT',
    data: {
      receiptFileKey,
      receiptNote: receiptNote || '骑手确认送达',
 ...
 */
async function deleteReceiptImage(mealSlotOrderId, receiptUrl) {
  try {
  ...
  // 调用后端API删除数据库记录
  return await request({
    url: `/api/rider/orders/${mealSlotOrderId}/receipt-image`,
    method: 'DELETE'
  });
}
 ...
 */
async function reportException(mealSlotOrderId, exceptionType, exceptionNote, exceptionImages) {
  return await request({
 ...
 * @returns {Promise<Object>} 操作结果
 */
async function revertOrderStatus(mealSlotOrderId) {
  return await request({
    url: `/api/rider/orders/${mealSlotOrderId}/revert`,
    method: 'POST'
  });
}
 ...
 * @returns {Promise<Object>} 操作结果
 */
async function undoDelivery(mealSlotOrderId) {
  return await revertOrderStatus(mealSlotOrderId);
}
 ...
 * @returns {Promise<Object>} 操作结果
 */
async function saveOrderSequence(batchItemIds) {
  return await request({
    url: '/api/rider/orders/reorder',
    method: 'POST',
    data: { batchItemIds },
    header: { 'content-type': 'application/json' }
  });
...
async function getAddressReferenceImage(addressId) {
  return await request({
    url: `/api/rider/address-reference?addressId=${addressId}`
  });
}
