...
public class OperationsAnalysisServiceImpl implements OperationsAnalysisService {
    ...
    @Override
    @Transactional
    public CreateCostEntryResponse createCostEntry(CreateCostEntryRequest request) {
        if (request.costDate() == null || request.costCategory() == null || request.amount() == null) {
        ...
        long id = insertAndReturnId(
            ...
            java.sql.Date.valueOf(request.costDate()),
            request.costCategory(),
            request.amount(),
            request.remark() == null ? "" : request.remark(),
            request.recordedBy() == null || request.recordedBy().isBlank() ? "后台客服" : request.recordedBy()
        );
        return new CreateCostEntryResponse(id, "CREATED");
    }
