...
class MobileMySqlCompatibilityTest {
    ...
    void resetMobileFixtures() {
        jdbcTemplate.update(
            "DELETE FROM menu_week_items WHERE week_id IN (SELECT id FROM menu_weeks WHERE week_start_date = DATE '2026-06-01')"
        );
        jdbcTemplate.update("DELETE FROM menu_weeks WHERE week_start_date = DATE '2026-06-01'");
        ...
        jdbcTemplate.update("DELETE FROM dispatch_assignments WHERE rider_profile_id >= 951");
        ...
        jdbcTemplate.update(
            """
                INSERT INTO meal_slot_orders (
                    ...
                    901, 901, 'LUNCH', 'LUNCH', 1, 901, '少饭', '少饭', 'DELIVERED', 'MINIAPP'
                )
                """
        );
        jdbcTemplate.update(
