...
> 使用方式：① 技能形态（**全局，所有项目通用**）`C:\Users\Lenovo\.codebuddy\skills\smart\SKILL.md`（状态机总控，自动路由，自动生效）② 本文档（完整底稿）。
...
8. **外接优先（find-skills 先行）**：新增任何理念/能力/流程前，先 `find-skills` 检索成熟热门现成技能，好用直接拿来用再优化；找不到才自研。**每个 S 流程审视/执行时都要 find-skills 查对应热门技能**（不要冷门，热门=大家用=值得借鉴；且必须核实内容，不能只看安装量）。**调研要彻底，不许假调研**：拆出领域全部子方向逐个搜，把热门技能全列出来说明"别人做了什么"，让老板判断吸收哪些，不管跟当前项目有没有关系（违反=敷衍=失败）
...
## S0 想法
- 触发：有了一个念头
- 技能：`grilling`（设计树轮次追问）+ `brainstorming`
- 执行：复述确认 → 对齐颗粒度（方向级/功能清单级/字段交互级）→ grilling 轮次追问（角色/功能/边界/定制需求；事实 AI 自查、决策问老板，每问带推荐答案）→ frontier 清空后直接写 PRD（产品需求，用户视角）
- 产出：`.scratch/<slug>/prd.md`（颗粒度约定 + 完整需求清单）
- 决策点：要不要继续？→ 问老板

## S1 验证
- 触发：想法确认想验证
- 技能：`idea-validator`（可行性）+ `bmad-research`（市场/竞品）+ 外接 `research`（技术向，带引用）
- 核心：算清**投入产出账**——钱/时间/精力/体力/脑力投入 vs 回报，不白花时间和钱
- 执行：按项目性质定验证维度（赚钱→市场/竞品/付费意愿/单位经济；自用→可行性"好不好做、成本多大、麻不麻烦"+收益"是否真提效/替代低效"）→ 针对 S0 核心假设逐条验证 → 算账给结论
- 产出：`.scratch/<slug>/validation.md`（假设结论 + 投入产出账 + 建议）
- 决策点：验证结论 → 问老板拍板

## S2 需求定义
- 触发：验证通过
- 技能：`to-spec`（→规格）+ `to-tickets`（切 ticket）+ `pricing`（若涉收费）
- 核心：**二八定律分阶段**——MVP 先做核心 20%，锦上添花排后 80%，不"既要又要"
- 执行：to-spec 把 PRD 转 spec（业务不变量/状态机/seam）→ 二八定律分阶段（MVP=核心 20%=一个用户最短核心链路，优化排后）→ 产出阶段计划 → to-tickets 切 MVP ticket
- 产出：`.scratch/<slug>/spec.md` + `.scratch/<slug>/roadmap.md`（分阶段）
- 决策点：MVP 边界 → 问老板
...
## S3 设计
- 触发：需求定了
- 技能：`frontend-design` + `prototype`（轻量可视化）+ `web-artifacts-builder`（复杂多组件原型）
- 核心：**可视化预览先行**——先出可预览文件，老板预览满意再写代码，绝不"写一点→启动→测→再看"
- 执行：复用已有设计系统 → 定设计方向（视觉+动画：灵动/酷炫 vs 严谨）→ 主动调研组件（find-skills 查成熟方案，先用别人的再优化）→ 定 token+线框 → 产出可预览原型 → 预览满意才写代码
- 产出：可预览原型（HTML）+ 设计 token 文件（固定位置）+ 设计方向说明
- 决策点：设计方向 → 给老板看，预览满意后执行

## S4 搭建骨架
- 触发：设计方向确认
- 技能：`codebase-design` + `improve-codebase-architecture` + `folder-structure-blueprint-generator`
- 核心：**架构师思维**——模块划分、解耦（改一处不动全身）、文档记录，不写屎山；从这步起凡涉及代码都要带此思维，不是写完再补救
- 执行：判断新/已有项目 → 技术选型（给依据）→ 选型后主动 find-skills 查该语言/框架热门技能（后端 Java→java-springboot、前端 React/Vue/小程序各有热门）→ 定 ARCHITECTURE.md（模块+串联）→ 目录（含 logs/）+ ADR → CI
- 产出：`ARCHITECTURE.md` + 目录结构 + 决策记录 + CI（跑测试）+ 迁移工具启用 + 测试框架
- 决策点：技术栈选型 → 问老板
...
## S5 数据库（防翻车核心）
- 触发：骨架就位
- 技能：`database-schema-designer` + `sql-code-review` + `sql-optimization`（SQL 性能）+ `mysql`（PlanetScale 官方）+ `database-migration`（迁移）
- 产出：schema + 迁移脚本 + **所有"至多一条"的 UNIQUE 约束** + 高频查询索引 + 唯一键设计理由注释
- 决策点：数据模型 → 问老板确认
...
8. 唯一键设计理由写进迁移注释
9. 迁移只增不删，上线后禁改历史迁移
10. 空值语义在 schema 层定死（字段可空性/默认值/NULL vs '' 边界写进注释）
11. 高频查询字段建索引（不只 UNIQUE 索引，按查询路径补普通索引）

## S6 开发（前后端都覆盖）
- 触发：schema 定了
- 技能：`tdd` + `semgrep` + `codebase-design` + `java-springboot`（Java 质量）
- 核心：**架构师思维执行**——写代码前先看 S4 的 ARCHITECTURE.md 模块划分，新代码归对应模块，保持解耦不引入新耦合（改一处不动全身）
- 产出：功能代码 + 测试 + 事务/并发/幂等实现
- 自检：写操作有事务吗？并发怎么办？幂等落库了吗？状态变更同步了吗？解耦了吗（改一处不动全身）？
- 决策点：一般不问，除非影响产品行为
...
### 前端规则
复用优先铁律（复用组件/不重复造轮子/新组件登记清单）+ 按语言调框架技能（React/Vue/小程序）
...
### 属性测试（吸收 property-based-testing，机制化极端场景）
- 可表达为不变量的规则（钱包守恒/终态不挂活跃派单/换区域一致性/金额精度/编码解码可逆）→ 属性测试机制化生成极端输入找反例，**不靠 AI 自觉往极端想**；无代数结构才用示例测试

### 接口级复杂流程测试（跨表一致性）
- 模拟用户走完整业务链路（下单→派单→送达→退款），连真实 DB 不 mock，验证跨表状态同步与一致性（换区域后关联表同步、终态订单不挂活跃派单）

## S8 审查
- 触发：测试通过
- 技能：`code-review` + `sql-code-review` + `adversarial-review`（状态机/资金/跨表必加）+ `grill-me`
- 产出：至少 2 个维度的审查结论 + 修复，**结论落 `.scratch/<slug>/review.md`（可追溯，不是只说"过了"）**
- 自检：只审了风格吗？一致性/并发/空值语义审了吗？注入/越权/泄露审了吗？
...
| 安全 | `semgrep` | 注入（SQL/XSS）/越权（横向纵向）/敏感数据泄露 |
...
- **容量规划（上线前必做，血的教训）**：预估并发用户量 → 配服务器内存/连接池/线程数 → 压测验证；**提前跟老板沟通"会有多少人用"，不配好不上线**
...
- **效果度量闭环**：度量结果反馈回 S2 需求（数据驱动迭代，不是一次性营销）
- 产出：营销内容 + 转化优化 + 效果度量
- 决策点：营销方向/预算 → 问老板
